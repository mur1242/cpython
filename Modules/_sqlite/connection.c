...
rtree_callback(sqlite3_rtree_query_info *info)
{
    ...
    parameters = _pysqlite_build_py_params(NULL,
                                           info->nParam,
                                           info->apSqlParam);
    if (parameters == NULL) {
        goto error;
    ...
    /* If this is the first call, set UDF context pointer and destructor.
     * The destructor will run when the last callback for this query is done. */
    if (info->pUser == NULL && context != NULL) {
        info->pUser = Py_NewRef(context);
        info->xDelUser = &_destructor;
    }
...
/*[clinic end generated code: output=bf3ae7bd13b4474f input=de379655c7d06223]*/
{
    ...
    if (callback == Py_None) {
        ...
        rc = sqlite3_rtree_query_callback(self->db,
                                          name,
                                          &rtree_callback,
                                          Py_NewRef(callback),
                                          &_destructor);
...
static PyMethodDef connection_methods[] = {
    ...
    PYSQLITE_CONNECTION_CREATE_RTREE_QUERY_FUNCTION_METHODDEF
